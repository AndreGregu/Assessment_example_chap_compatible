...
class PeakValueDiffMetric(MetricBase):
    spec = MetricSpec(
        ...
        description="Difference in peak magnitude: truth_peak_value - pred_peak_value (per location).",
    )
...
# =========================================
# Metric 2: Peak week lag (weeks difference)
# =========================================
...
if __name__ == "__main__":
    ...
    peak_value_diff_metric = PeakValueDiffMetric()
    peak_value_diff_result = peak_value_diff_metric.get_metric(flat_observations, flat_forecasts)

    peak_lag_metric = PeakWeekLagMetric()
    peak_lag_result = peak_lag_metric.get_metric(flat_observations, flat_forecasts)

    print("\nPeak Value Difference (per location):")
    print(peak_value_diff_result)

    print("\nPeak Month Lag (per location):")
    print(peak_lag_result)
